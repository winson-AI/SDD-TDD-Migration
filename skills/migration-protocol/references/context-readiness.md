...
宿主以已认证 role/instance/module 启动或恢复**只读预检**，允许读获授权材料、写自身 staging、提交 context-submit；未获原有 assign/freeze 等门禁批准，不允许改目标代码或执行目标测试。preflight 和正式执行必须使用同一实例；换实例必须重新核对。宿主继续承担身份认证、Used Skills 加载、工具可用性和文件写隔离。
...
- `execution`：testing/audit-testing 的 ready 报告必填 argv、绝对 cwd、environment_ref；说明工具版本、构建、设备/服务、fixtures/账号可用性、seed、重置/清理方法。秘密值不入报告。
...
blocked 报告提交会保留缺失项，但**不会自动将整个 MO 标记收尾**。责任编排者必须继续补材料，或基于本模块实际原因提交已有 suspend（dependency/human/tooling）；已确认依赖/外围问题符合 audit-defer 条件时走该分支。审计修复中的阻塞经 audit-block 进入相关 finding 的人工报告。缺失不伪造失败测试，未执行路径仍为未执行；保留 Yellow 原因和恢复条件。
