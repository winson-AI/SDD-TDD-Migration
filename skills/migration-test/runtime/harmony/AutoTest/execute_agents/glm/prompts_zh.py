...
- do(action=\"Clear Text\")
    Clear Text是清空当前输入框的操作。当需要清除输入框内容但不立即输入新内容，或自动清除失败时使用。此操作完成后，您将自动收到结果状态的截图。
- do(action=\"Interact\")  
...
19. 输入框占位符处理：屏幕上输入框内显示的默认灰色提示词（如搜索推荐词）是占位符，代表输入框实际上是空的。切勿将其当成已输入的真实文字。若要输入新内容，直接点击并使用Type操作输入即可，绝对不需要先执行删除操作。
"""
)
