...
埋点不作为第五维：实际触发入口归 UI，事件/参数/时机/副作用归 Logic，SDK/DI/生命周期/平台桥接归 Adhesive，实际配置归 Resource。GO 划模块后检查，父 MO 划子模块后分配，子 MO 划任务后明确职责。无埋点不使整个四维分析失效，也不需要创造埋点条目；有埋点任务可与普通 N/A 任务并存。规则及事件映射见 [埋点协议](telemetry.md)。
